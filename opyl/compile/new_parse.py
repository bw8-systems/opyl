...
from .lexemes import GroupingPair
...
class OpalParser(comb.BaseTokenParser):
    def constant_declaration(self) -> nodes.ConstDeclaration:
        ...
        (((keyword, identifier), type), initializer), newline = parse(
            ...
            .followed_by(self.primitive[pk.Colon])
            .and_also(self.type)
            .followed_by(self.primitive[pk.Equal])
            .and_also(self.expression)
            .and_also(self.primitive[pk.NewLine])
        )

        # self.keyword(kk.Const).identifier().consume(pk.Colon).type().consume(
        #     pk.Equal
        # ).expression().primitive(pk.NewLine)
    ...
    def variable_declaration(self) -> nodes.VarDeclaration:
        ...
        (((((keyword, maybe_mut), identifier), type), initializer), newline) = parse(
            self.keyword[kk.Let]
            .and_also(self.one_or_none(self.keyword[kk.Mut]))
            .and_also(self.identifier)
            .followed_by(self.primitive[pk.Colon])
            .and_also(self.type)
            .followed_by(self.primitive[pk.Equal])
            ...
            .and_also(
                this=self.between_pair(
                    ...
                    between=self.zero_or_more(
                        comb.Lift(self.field).followed_by(pk.NewLine)
                    ).and_also(
    ...
    def union_declaration(self) -> nodes.UnionDeclaration:
        ...
        keyword = parse(self.keyword[kk.Union])
        identifier = parse(self.identifier)

        generic_specs = parse(self.generic_spec)
        trait_bases = parse(self.trait_bases)

        parse(self.primitive[pk.Equal])

        members = [parse(self.type)]
        members.extend(parse(self.one_or_more(self.primitive[pk.Pipe].then(self.type))))
    ...
    def enum_declaration(self) -> nodes.EnumDeclaration:
        ...
        keyword = parse(self.keyword[kk.Enum])
        identifier = parse(self.identifier)

        members = [parse(self.primitive[pk.LeftBrace].then(self.identifier))]
        members.extend(
            parse(self.zero_or_more(self.primitive[pk.Comma].then(self.identifier)))
        )

        right_brace = parse(self.primitive[pk.RightBrace])
    ...
    def trait_declaration(self) -> nodes.TraitDeclaration:
        ...
        # keyword = parse(self.keyword[kk.Trait])
        # identifier = parse(self.identifier)
    ...
    def paramspec(self) -> nodes.ParamSpec:
        ...
        # Syntax:
        # anon name: type,
        anon = parse(self.one_or_none(self.keyword[kk.Anon]))
        identifier = self.identifier()
        parse(self.primitive[pk.Colon])
        param_type = self.type()
        comma = parse(self.primitive[pk.Comma])
    ...
    def function_signature(self) -> nodes.FunctionSignature:
        ...
        keyword = parse(self.keyword[kk.Def])
        identifier = self.identifier()

        _left, right, params = parse(
            self.between(
                open=self.primitive[pk.LeftParenthesis],
                close=self.primitive[pk.RightParenthesis],
                between=self.zero_or_more(self.paramspec),
            )
        )

        return_type = None
        return_arrow = parse(self.one_or_none(self.primitive[pk.RightArrow]))
    ...
    #     generic_params = []
    #     if (
    #         parse(self.one_or_none(self.primitive[pk.LeftBracket]))
    #         is not None
    #     ):
    ...
    #                 )
    #             ).then(self.primitive[pk.RightBracket])
    #         )

    #     trait_bases = parse(
    #         self.one_or_none(self.primitive[pk.LeftParenthesis])
    #         .then(
    #             self.zero_or_more(
    #                 self.identifier.followed_by(self.primitive[pk.Comma])
    #             )
    #         )
    #         .followed_by(self.primitive[pk.RightParenthesis])
    #     )
